"""Update & deployment model (spec 9).
  ...
  * Failed attempts retry automatically when the PC is idle (agent-side, same idle mechanism as
...
async def report_status(ctx: Context, payload: dict[str, Any]) -> dict[str, Any]:
    """Agent reports {version, status: ok|failed|retrying, attempts}. Past-threshold failures
