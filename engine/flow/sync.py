...
The Engine coordinates; the actual file moves/transforms run on the worker client that owns the
...
async def propagate(engine: Engine, flow: dict[str, Any], event: dict[str, Any]) -> None:
    """Walk stages; dispatch per-destination sync commands to the owning worker client; log the sync;
...
async def destination_write(ctx: Context, payload: dict[str, Any]) -> dict[str, Any]:
    """Worker Client reports a write observed at a flow destination with its attribution, so the Engine
...
async def sync_result(ctx: Context, payload: dict[str, Any]) -> dict[str, Any]:
    """Worker Client reports the outcome of a sync command: {"flow_id", "branch_id", "status",
